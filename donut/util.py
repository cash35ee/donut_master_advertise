...
class DonutDataset(Dataset):
    ...
    def _load_local_dataset(self, dataset_name_or_path, split):
        print(f"Dataset base path: {dataset_name_or_path}")
        if split == "train":
            json_file = os.path.join(dataset_name_or_path, "train", "metadata.jsonl")
        elif split == "validation":
            json_file = os.path.join(dataset_name_or_path, "validation", "metadata.jsonl")
        else:
            raise ValueError(f"Unknown split: {split}")
        
        print(f"Attempting to open JSONL file: {json_file}")
        
        if not os.path.exists(json_file):
            raise FileNotFoundError(f"Could not find the JSONL file: {json_file}")
    ...
    def __init__(
        ...
        self.decoder = self.donut_model.decoder  # 設置 self.decoder
        ...
        self.dataset = self._load_local_dataset(dataset_name_or_path, split)
        self.dataset_length = len(self.dataset)

        self.gt_token_sequences = []
        for sample in self.dataset:
            ground_truth = json.loads(sample["ground_truth"])
            if "gt_parses" in ground_truth:  # 當有多個 ground truth 時
        ...
        # 添加特殊 tokens
        if self.task_start_token and self.prompt_end_token:
            self.decoder.add_special_tokens([self.task_start_token, self.prompt_end_token])
            self.prompt_end_token_id = self.decoder.tokenizer.convert_tokens_to_ids(self.prompt_end_token)

    def __len__(self) -> int:
        return self.dataset_length

    def __getitem__(self, idx: int) -> Union[Tuple[torch.Tensor, torch.Tensor, torch.Tensor], Tuple[torch.Tensor, torch.Tensor, torch.Tensor, str]]:
        """
        Load image and convert gt data into input_ids.
        """
        sample = self.dataset[idx]
        if 'file_name' not in sample:
            print(f"Sample {idx} missing 'file_name' key: {sample}")
        ...
        try:
            ...
            print(f"Error loading image {sample['file_name']}: {e}")
            raise e
